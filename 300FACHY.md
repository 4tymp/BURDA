# Fachy

Fachy to coś, czym można by było nazwać twoją postać. Są dosyć trudne do odblokowania, ale dają znaczącą przewagę w świecie BURDY. Możesz mieć ich nieograniczoną ilość.

Maksymalnie można śledzić 5 Fachów, które chcemy odblokować. Dopiero podczas śledzenia danego Fachu są zaliczane wymagania.

Jeżeli chcemy zmienić śledzony Fach na inny, postęp wcześniejszego przepada.

# Krzepkie:

## Kuba \!\!\!

Wymagania:

* Rębajło: 40  
* Kunszt Bryk na 3 poziomie.  
* Wykonaj 15 razy kontratak

Zdolność:

Po zostaniu zaatakowanym możesz zbliżyć się do atakującego za max połowę ruchu i zaatakować go, jeśli jest w zasięgu.

## Łapiger

Wymagania:

* Rębajło/Dziurawiec: 45  
* Wykonanie 10 zleceń  
* Lekka rozpoznawalność w świecie gry.

Zdolność:

Możesz wybrać jednego przeciwnika na początku walki. Zostaje on twoim Celem.

Jeżeli wykonujesz zlecenie i walczysz z przedmiotem zlecenia, automatycznie zostaje on twoim Celem.

Gdy Łapiger atakuje cel:

1. Dostaje kość Triumfu na testy Ataku.  
2. Jego szansa na Ikrę zwiększa się dwukrotnie.  
3. Wszystkie jego ataki nie są śmiertelne.

## Ręb

Wymagania:

* Rębajło: 60  
* Tężyw: 60  
* Kunszt Chyżość na 4 poziomie  
* 5 udanych testów Woli Walki ze zniszczonymi co najmniej 3 kończynami  
* Wyekwipowany lekki pancerz lub brak pancerza

Zdolność:

Za każdą Zniszczoną kończynę Ręb dostaje dodatkową BURDĘ.

Dodatkowo, po nieudanym teście Woli Walki Ręb od razu przejmuje Turę przeciwnika, w której ma wszystkie swoje BURDY, wszystkie Ataki to Zniszczenie, nie działają na niego kary za Zranione i Zniszczone Kończyny, oraz nic nie może zadać mu obrażeń. Po zakończeniu tej tury, czyli wykorzystaniu wszystkich BURD, Ręb traci przytomność.

## Trypuł

Wymagania:

* Rębajło: 60  
* Erudyta: 40  
* Kunszt Letarg  
* Zabić krytycznie przeciwnika oraz zaatakować sojusznika w tej samej turze.

Zdolność:

Zaczyna działać mechanika Szału. Berserk nie może stracić przytomności podczas jej działania.

Berserk musi zadeklarować wejście w szał Kosztuje to jeden punkt Wigoru.

Szał rośnie, gdy:

* Postać trafi atakiem: \+2  
* Postać trafi Ikrą: \+4  
* Postać zostanie zaatakowana: \+1

Szał spada, gdy:

* Postać nikogo nie zaatakuje w trakcie tury: \-5  
* Postać spudłuje atak: \-1  
* Postać wyrzuci Licho: \-3

Gdy szał spadnie do 0, Berserk wychodzi z szału i traci wszystkie punkty Wigoru.

Bonusy za szał:

* 5: Dodatkowa Reakcja  
* 10: Jeden typ obrażeń więcej  
* 15: Dodatkowa BURDA  
* 20: Jeden typ obrażeń więcej  
* 25: Dodatkowa BURDA

Nie ma górnego limitu szału.

## Dusza Demona

Wymagania:

* Rębajło: 60  
* Tężyw: 60  
* Kunszt Krzepa na 4 poziomie  
* Kunszt walki dowolną bronią dwuręczną

Zdolność:

\+15 do testów Tężywa

Potrafi używać broń dwuręczną w jednej ręce.

## Myślorąb

Wymagania:

* Rębajło: 40  
* Erudyta: 50  
* Kunszt Baczenie  
* Użyj Baczenia krytycznie 4 razy.  
* Wyekwipowany lekki pancerz lub brak pancerza

Zdolność:

Raz na turę możesz wykonać test Erudyty, aby dodać sobie dodatkowe k4 BURDY do końca tury.

## Gruziłło

Wymagania:

* Rębajło: 40  
* Tężyw: 60  
* Kunszt Ciężki Pancerz na 2 poziomie  
* Wyekwipowany Ciężki Pancerz

Zdolność:

Niwelowane są kary do Ruchu za segmenty ciężkiego pancerza. ?\!?\!?\!?\!?\!?\!?\!?\!?\!?\!?

Leczenie Gruziłła jest o jeden poziom skuteczniejsze.

Gruziłło może wykonać rzut obronny zamiast sojusznika znajdującego się w Bliskim zasięgu od niego, aby go obronić. Kosztuje go to oczywiście 1 Reakcję. Jeśli rzut się nie powiedzie, obrażenia spadają na Gruziłło.

## Orężnik

Wymagania:

* Rębajło: 60  
* Kunszt walki co najmniej 5 brońmi.

Zdolność:

Nie posiadasz już kar za używanie broni, w której nie masz Kunsztu.

Dostajesz dodatkowy atak, który nie zużywa punktu BURDY.

Sojusznicy w Bliskim zasięgu od Orężnika dostają \+10 do szansy na trafienie bronią.

## Brzdąk

Wymagania:

* Rębajło: 40  
* Tężyw: 30  
* Kunszt Bajera  
* Zdobycie części ekwipunku co najmniej 4 jakości.

Zdolność:

Rzut Brzdąka na BURDĘ jest liczony zawsze jako Wynik \-20.

Jeżeli walczy z przeciwnikiem sam na sam, posiada: \+10 do Rębajło, \+5 szansy na trafienie krytyczne, oraz jego obrażenia są o jeden poziom wyższe.

## Zasłonnik

Wymagania:

* Tężyw: 60  
* Rębajło: 40  
* Kunszt Walka Tarczą  
* Sparowanie 20 ciosów tarczą.

Zdolność:

Sojusznicy Zasłonnika dostają \+10 do rzutów obronnych, gdy walczą wraz z nim.

Za każdym razem, gdy Tarczownik atakuje bronią inną niż tarczą, może za darmo dodać obrażenia tarczy do ataku.

Może wykonać ripostę po udanym parowaniu, która zadaje obrażenia tarczy.

# Chytre:

## Noży Strzelec \!\!\!

Wymagania:

* 

Zdolność:

## Hultaj

Wymagania:

* Chybki: 40  
* Kunszt Szmuglerka  
* 15 udanych kradzieży

Zdolność:

Może sprowokować wroga testem Chybkiego do końca walki.

Atakując sprowokowanego wroga jego szansa na krytyk jest zwiększona o 10\.

## Kmieć\!\!

Wymagania:

* 

Zdolność:

## Burżuj\!\!

Wymagania:

* 

Zdolność:

## KUMITSU \!\!\!

## Wymagania:

* 

Zdolność:

# Magiczne:

## Prawdziwy Mnich

Wymagania:

* Szmagier: 50  
* Erudyta: 60  
* Kunszt Magia Pierwotna na 3 poziomie  
* Umiejętność Letarg  
* 10 razy medytuj podczas walki z co najmniej 3 wrogami

Zdolność:

Posiadasz 1,5 razy więcej Jaźni zaokrąglone w dół. Medytowanie przywraca ci pełnię Jaźni.

## Mag Run\!\!\!

Wymagania:

* Szmagier: 40  
* Erudyta: 40  
* Kunszt Inskrypcja  
* Kunszt Piśmienność  
* Kunszt Magia (dowolna)  
* Stwórz 3 nietypowe Grimuary.

Zdolność:

Potrafisz rzucać zaklęcia runiczne podobnie do zwykłych zaklęć\! W tym przypadku maksymalna ilość run w zaklęciu 2 razy poziom umiejętności Inskrypcja.

## Świstak

Potrafi zamieniać gwizdy oraz dmuchnięcia w ostre jak brzytwa klingi latające po polu bitwy. Jeśli się postara, potrafi zabrać sluch każdemu w sporym mieście… wszystko jednym gwizdnięciem.

Wymagania:

* Szmagier: 40  
* Kwiotkowy: 30  
* Umiejętność Magia Pierwotna  
* Umiejętność Magia Żywiołów: Powietrznik  
* Umiejętność Skupienie  
* Umiejętność Muzyka  
* Zagranie piosenki dla tłumu bez instrumentu i śpiewu, używając wyłącznie magii.

Zdolność:

\+10 do testów Kwiotkowego

O ile jesteś w stanie otworzyć usta możesz gwizdem tworzyć powietrzne ostrza. Są one domyślnie traktowane jak obrażenia miecza jednoręcznego i są traktowane jako Atak. Oczywiście nic nie stoi na przeszkodzie, aby przy użyciu Many ulepszyć je jak zwykłe zaklęcia.

## Mag Czasu

Wymagania:

* Szmagier: 40  
* Umiejętność Magia Pierwotna  
* Posiadanie Zegarka  
* Zużycie 50 Przerzutów  
* Udane cofnięcie się w czasie

Zdolność:

\+5 do testów Magii Pierwotnej

Mag czasu raz dziennie może powtórzyć dowolny test. W zamian za wszystkie swoje Punkty Many potrafi wykorzystać czas swojego przeciwnika, aż do jego śmierci. Niestety on sam kończy po tym ze Zniszczonymi organami wewnętrznymi.

## Prawdziwy Mag Pierwotny

Wymagania:

* Szmagier: 60  
* Erudyta: 60  
* Umiejętność Magia Pierwotna na 4 poziomie  
* Stary wiek, w większości poświęcony nauce o Magii Pierwotnej

Zdolność:

\+20 do testów Magii Pierwotnej

Prawdziwemu Magowi Pierwotnemu nigdy nie dzieje się Pech. Dodatkowo, nie musi wykonywać Testów Szmagiera, wszystkie są zawsze rozpatrzone pozytywnie. Nie ma on żadnych Akcji Obronnych oprócz jego własnej \- cofnięcia się w czasie oraz ucieknięcia w bezpieczne miejsce turę wcześniej. Może tak zrobić 3 razy podczas rundy. Mag Pierwotny nie potrafi się denerwować, ponieważ i tak widział juz wszystko wszedzie i zawsze milion razy. W tych czasach żyje, dopóki mu się nie znudzą i przejdzie do następnych.

## Antymag

Antymagowie to osoby parający się magią pierwotną. Używają ją w celu rozpraszania Magii oraz walczenia z osobami magicznymi.

Wymagania:

* Szmagier: 50  
* Rębajło/Dziurawiec: 40  
* Erudyta: 30  
* Umiejętność Magia Pierwotna  
* Umiejętność Odparcie na 3 poziomie  
* Nałożenie efektu Uciszenie 10 razy  
* 5 Udanych odparć magii

Zdolność:

Antymag ma bonus \+10 do wszystkich testów, których celem są osoby Magiczne.

Potrafi spalić k4 punktów Many celu testem Szmagiera. Cel musi znajdować się w zasięgu 4m. Przez tyle tur, ile punktów Many celu spalił jego ataki brońmi będą zadawać jeden stopień obrażen więcej oraz będą uciszały cel na 1 turę.

Jeżeli spali ostatni punkt Many celu, Antymag posiada zawsze 50% na wykończenie celu jednym ciosem.

## Konstelator

Konstelator to mag pierwotny, który miłuje się w gwiazdach.

Wymagania:

* Szmagier: 50  
* Umiejętność o Historii  
* Pozyskana wiedza o Astronomii

Zdolność:

\+10 do testów Szmagiera. \+20 gdy Konstelator czaruje pod gołym nocnym niebem.

Konstelator potrafi zaczerpnąć moc z gwiazd, aby zwiększyć moc swoich czarów, niwelując karę za ulepszenie zaklęcia do \-30. Kosztuje to 1 pkt. Kondycji.

## Wieszcz

Wieszcz to osoba potrafiąca widzieć przyszłość.

Wymagania:

* Szmagier: 60  
* Erudyta: 50  
* Umiejętność Historia na 3 poziomie  
* Umiejętność Magia Pierwotna

Zdolność:

Wieszcz posiada dwa razy więcej kości inicjatywy.

Potrafi przewidzieć niedaleką przyszłość w zamian za połowę many, lub wyczytać przyszłość danej osoby za jej pełnię.

W zamian za test Szmagiera, 1 pkt. Many i Kondycji jest nie do trafienia przez ataki przez k4 tur.

## Dziadek mróz

Wymagania:

* Szmagier: 40  
* Umiejętność Magia Żywiołów: Wodnik  
* Umiejętność Skupienie na 3 poziomie  
* Medytacja w mroźnym miejscu przez 7 dni razem.

Zdolność:

\+10 do testów Magii Żywiołów: Wodnik

Dziadek mróz potrafi natychmiastowo zmienić wodę w lód i odwrotnie. Każda jego akcja ma dodatkowe 50% na efekt Odmrożenie oraz 25% na Spowolnienie.

Dodatkowo leczenie magią Wodnika zawsze jest o jeden stopień lepsze.

## Troll

Wymagania:

* Szmagier: 40  
* Umiejętność Magia Żywiołów: Ziemnik  
* Umiejętność Skupienie na 3 poziomie  
* Medytacja w głębokim miejscu przez 7 dni razem.

Zdolność:

\+10 do testów Magii Żywiołów: Ziemnik

Zbroja Trolla trwa teraz tak długo, aż ktoś jej nie zniszczy lub sam Troll jej nie odwoła. Każda jego akcja ma 25% na nałożenie efektu Powalenie.

Raz na turę potrafi wykonać darmową akcję PIACH W OCZY, która ma 50% na oślepienie przeciwnika. Cały czas liczy się ona jako Manewr.

## Wiatr

Wymagania:

* Szmagier: 40  
* Umiejętność Magia Żywiołów: Powietrznik  
* Umiejętność Skupienie na 3 poziomie  
* Medytacja w wysokim miejscu przez 7 dni razem.

Zdolność:

\+10 do testów Magii Żywiołów: Powietrznik

Wiatr płaci 1pkt Many za wejście w stan Lewitacji, po czym lewituje tak długo, jak mu się podoba. Każda jego akcja ma 25% na nałożenie efektu Dezorientacja. Dodatkowo, przy każdej akcji może za darmo odepchnąć lub przyciągnąć cel za dziesiątke cechy Szmagier metrów (plus bonusy).

## Demon

Wymagania:

* Szmagier: 40  
* Umiejętność Magia Żywiołów: Ognik  
* Umiejętność Skupienie na 3 poziomie  
* Spalenie conajmniej małej wioski raz z jej mieszkańcami

Zdolność:

\+10 do testów Magii Żywiołów: Ognik

Wszystkie ataki Demona mogą być traktowane jako obszarówka 4x4m. Demon zadaje obrażenia o jeden poziom większe każdemu przeciwnikowi, który jest podpalony. Każda jego akcja ma 25% na nałożenie efektu Podpalenie.

## Elementalista

Mistrz magii żywiołów, który okiełznał wszystkie żywioły na mistrzowskim poziomie.

Wymagania:

* Umiejętność Magia Żywiołów: Wodnik na 4 poziomie  
* Umiejętność Magia Żywiołów: Ziemnik na 4 poziomie  
* Umiejętność Magia Żywiołów: Powietrznik na 4 poziomie  
* Umiejętność Magia Żywiołów: Ognik na 4 poziomie

Zdolność:

\+20 do testów Magii Żywiołów.

Elementalista może łączyć dwa wybrane żywioły w jednym zaklęciu.

Dodatkowo, z żywiołu:

* Wody: Wszystkie efekty trwają k4 tur więcej.  
* Ziemi: Zyskuje odporność na efekty równą cesze Szmagier  
* Wiatru: Dostaje dodatkową Akcję  
* Ognia: Dostaje dodatkową kość k10 do inicjatywy.

## Nekromanta

Wymagania:

* Szmagier: 40  
* Umiejętność Czarna Magia  
* Przywołanie do życia 15 nieumarłych kreatur.  
* Umiejętność Medycyna

Zdolność:

Za każdym razem, gdy kreatura Nekromanty zostanie zniszczona/odpędzona, Nekromanta dostaje 1pkt. Many.

Nekromanta może przywoływać trupy z ziemi. Koszt za k4 trupy pierwszego poziomu to tyle samo, co przywołana kreatura 1 poziomu z tabeli tworzenia kreatur. Identycznie wygląda to z następnymi poziomami. Każdy trup zostaje zniszczony po 1 ciosie.

## Ciemny Mag 2 \!\!\!

Wymagania:

* 

Zdolność:

## Ciemny Mag 3 \!\!\!

Wymagania:

* 

Zdolność:

## Ciemny Mag 4 \!\!\!

Wymagania:

* 

Zdolność:

## Ciemny Mag 5 \!\!\!

Wymagania:

* 

Zdolność:

## Jasny Mag 1 \!\!\!

Wymagania:

* 

Zdolność:

## Jasny Mag 2 \!\!\!

Wymagania:

* 

Zdolność:

## Jasny Mag 3 \!\!\!

Wymagania:

* 

Zdolność:

## Jasny Mag 4 \!\!\!

Wymagania:

* 

Zdolność:

## Jasny Mag 5 \!\!\!

Wymagania:

* 

Zdolność:

# Inne

## Jednokończyny

Wymagania:

* Brak jednej kończyny (do pary) przez 7 dni.  
* Brak protezy w miejscu brakującej kończyny.

Zdolność:

Nauczyłeś się żyć z jedną kończyną mniej. Aby zrekompensowac brak kończyny do pary, to co ci zostało pracuje lepiej. Wszystkie testy wykonywane tą kończyną mają \+10.

## Bezręki

Wymagania:

* Brak rąk przez conajmniej 7 dni.  
* Brak protez potrafiących zastąpić ręce.

Zdolność:

Nauczyłeś się żyć bez rąk, a ich stare funkcje przejęły teraz nogi. Możesz wykonywać wszystkie akcje bez kar za brak kończyn. Dodatkowo twoje nogi stały się mocniejsze. Dostajesz umiejętność Aktywny. Niestety, w zwarciu możesz posługiwać się tylko bronią małą oraz walką wręcz. Do testów walki wręcz dostajesz bonus \+30.

## Beznogi

Wymagania:

* Brak nóg przez conajmniej 7 dni.  
* Brak protez potrafiących zastąpić nogi.

Zdolność:

Nauczyłeś się żyć bez nóg, a ich stare funkcje przejęły teraz ręce. Możesz wykonywać wszystkie akcje bez kar za brak kończyn. Dodatkowo twoje ręce stały się mocniejsze. Dostajesz \+20 do testów cechy Tężyw oraz zadajesz jeden typ obrażen więcej, ale masz też \-20 na testy cech Rębajło oraz Dziurawiec.

## Zwierzy język

Wymagania:

* Erudyta: 40  
* Umiejętność Tresura  
* Spędzenie co najmniej tygodnia w dziczy w towarzystwie jedynie zwierząt.

Zdolność:

Potrafisz rozmawiać z większością bestii i zwierzaków\!

## Wąpierz \!\!\!

Wampir, krwiopijca, potwór.

Wymagania:

* Zarażenie wampiryzmem

Zdolność:

## Likantrop \!\!\!

Wilkołak, dzikus, potwór.

Wymagania:

* zostanie przeklętym jako wilkołak

Zdolność:

## Cierpliwy Nauczyciel

Wymagania:

* Erudyta: 60  
* 15 Umiejętności

Zdolność:

Osoba, którą uczysz potrzebuje zaledwie 5 udanych sesji nauczania zamiast 10\.
